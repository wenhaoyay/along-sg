...
export function SpatialMap({ origin, destination, stops = [], baselineGeometry = [], routeGeometry = [], activeStop, onStopSelect }: { origin: ResolvedLocation | null; destination: ResolvedLocation | null; stops?: Stop[]; baselineGeometry?: Coordinate[]; routeGeometry?: Coordinate[]; activeStop?: number | null; onStopSelect?: (index: number) => void }) {
  ...
  useEffect(() => {
    ...
    void import("leaflet").then((L) => {
      ...
      if (origin) allMarkersRef.current.push(marker(origin.coordinate, "origin", origin.label, "A"));
      ...
      if (destination) allMarkersRef.current.push(marker(destination.coordinate, "destination", destination.label, "B"));
      ...
      window.setTimeout(() => { mapRef.current?.invalidateSize(); spreadColliding(mapRef.current, allMarkersRef.current); }, 50);
    });
...
function spreadColliding(
  ...
  markers.forEach((marker) => {
    ...
    if (el) { el.style.removeProperty("--fan-x"); el.style.removeProperty("--fan-y"); }
  });

...
async function commonRoutes(page: Page) {
  ...
  await page.route("**/api/catalog/categories", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify([{ slug: "fried_chicken", name: "Fried chicken", parent_slug: "fast_food", outlet_count: 125 }, { slug: "japanese_food", name: "Japanese food", parent_slug: "food_drink", outlet_count: 44 }]) }));
  await page.route("**/api/catalog/search**", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify([{ display_name: "Sakura Independent Kitchen", kind: "place", canonical_brand: null, categories: ["japanese_food"], outlet_count: 1, example_location: "Clementi" }]) }));
  await page.route("**/api/discovery/needs**", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ query: "Japanese", semantic_type: "category", canonical_concept: null, category: null, confidence: "likely", related_terms: [], places: [], live_fallback_used: false }) }));
  await page.route("**/api/geocode**", (route) => { const url = new URL(route.request().url()); const q = url.searchParams.get("q")?.toLowerCase() ?? ""; const matches = q.includes("chua") ? [cck, { ...cck, label: "Choa Chu Kang Bus Interchange" }] : q.includes("fajar") ? [fajar] : q.includes("orchard") ? [orchard] : []; return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ query: q, results: matches, provider: "mock" }) }); });
}
...
test("partial requests require consent and mobile sheets resize", async ({ page, isMobile }) => {
  ...
  await page.route("**/api/intent/parse", (route) => route.fulfill({ json: { status: "resolved", intent, unresolved_terms: ["shoe repair"] } }));
  let calls = 0;
  await page.route("**/api/optimize-intent", (route) => { calls++; return route.fulfill({ json: result }); });
  ...
  expect(calls).toBe(1);
  await expect(page.getByText(/Added time includes/)).toBeVisible();
...
test("scheduled departure uses Singapore offset and stop selection stays in sync", async ({ page }) => {
  await commonRoutes(page);
  await page.route("**/api/intent/parse", (route) => route.fulfill({ json: { status: "resolved", intent } }));
  let departure: string | null = null;
  await page.route("**/api/optimize-intent", (route) => { departure = route.request().postDataJSON().departure; return route.fulfill({ json: result }); });
  await page.goto("/"); await resolveJourney(page);
  ...
  await stop.click(); await expect(stop).toHaveAttribute("aria-pressed", "true");
  await expect(page.locator(".along-marker.stop.selected-stop")).toHaveCount(1);
});

test("cancelled searches cannot replace a later result and preserve the journey", async ({ page }) => {
  await commonRoutes(page);
...
test("failed discovery offers explicit related searches without claiming stock", async ({ page }) => {
  await commonRoutes(page);
  await page.route("**/api/catalog/search**", (route) => route.fulfill({ json: [] }));
  await page.route("**/api/discovery/needs**", (route) => route.fulfill({ json: { semantic_type: "product", places: [], related_terms: ["printer ink", "printer supplies"] } }));
  await page.goto("/");
  ...
  await expect(page.getByLabel("Search brands, shops or categories")).toHaveValue("printer supplies");
});
...
test("failed comparison can be retried without reentering the journey", async ({ page }) => {
  ...
  await page.route("**/api/intent/parse", (route) => route.fulfill({ json: { status: "resolved", intent } }));
  let calls = 0;
  await page.route("**/api/optimize-intent", (route) => { calls++; return calls === 1 ? route.fulfill({ status: 503, json: { detail: "busy" } }) : route.fulfill({ json: result }); });
  await page.goto("/"); await resolveJourney(page);
  await page.getByLabel("What do you need on the way?").fill("KFC");
  await page.getByRole("button", { name: "Find best stop" }).click();
...
test("journey swap and walking preference reach the optimizer", async ({ page }) => {
  await commonRoutes(page);
  await page.route("**/api/intent/parse", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ status: "resolved", intent }) }));
  let submitted: { origin: { coordinate: { latitude: number } }; intent: { preferences: { walking_tolerance: string } } } | undefined;
  ...
  await expect(page.getByRole("button", { name: "Less walking", exact: true })).toHaveAttribute("aria-pressed", "true");
  await page.getByLabel("What do you need on the way?").fill("KFC");
  await page.getByRole("button", { name: "Find best stop" }).click();
  await expect(page.getByRole("heading", { name: "Lot One" })).toBeVisible();
...
test("resolved journey produces a hub-first map recommendation", async ({ page }) => {
  ...
  await expect(page.getByText("Extra travel")).not.toBeVisible();
  await page.getByText("Why this option").click();
  await expect(page.getByText("Extra travel")).toBeVisible();
  await expect(page.getByText("Other options")).toHaveCount(0);
  await expect(page.locator("body")).not.toContainText(/routing calls|provider latency|candidate count|best_match|hard constraints/i);
  ...
  if (test.info().project.name === "mobile-chromium") {
    const sheet = await page.getByRole("complementary", { name: "Recommendation", exact: true }).boundingBox();
...
test("different alternatives stay collapsed until requested", async ({ page }) => {
  await commonRoutes(page);
  await page.route("**/api/intent/parse", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ status: "resolved", intent, diagnostics: {}, journey_mentions: [], journey_conflicts: [] }) }));
  await page.route("**/api/optimize-intent", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ ...result, recommendations: { best_overall: recommendation, least_walking: walkingAlternative } }) }));
  await page.goto("/"); await resolveJourney(page);
  await page.getByLabel("What do you need on the way?").fill("KFC and bubble tea");
  await page.getByRole("button", { name: /Find best stop/ }).click();
...
test("location autocomplete supports keyboard selection", async ({ page }) => {
  await commonRoutes(page); await page.goto("/");
...
test("destination conflict blocks optimization until a user chooses", async ({ page }) => {
  await commonRoutes(page); let optimizeCalls = 0;
  await page.route("**/api/intent/parse", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ status: "needs_clarification", intent: { ...intent, original_text: "bubble tea" }, clarification_question: "Your destination above is Fajar LRT Station, but your request mentions Orchard MRT.", diagnostics: {}, journey_mentions: [], journey_conflicts: [{ endpoint: "destination", current_label: "Fajar LRT Station", mentioned_text: "Orchard MRT", mentioned_label: "Orchard MRT", latitude: 1.3043, longitude: 103.8322, reason: "Your destination above is Fajar LRT Station, but your request mentions Orchard MRT." }] }) }));
  await page.route("**/api/optimize-intent", (route) => { optimizeCalls += 1; return route.abort(); });
  await page.goto("/"); await resolveJourney(page);
...
test("non-errands are explained and the structured catalog is live", async ({ page }) => {
  await commonRoutes(page);
  await page.route("**/api/intent/parse", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ status: "unresolved", intent: null, clarification_question: "I couldn't find an errand in that request. Try ‘KFC and bubble tea’.", diagnostics: {}, journey_mentions: [], journey_conflicts: [] }) }));
  await page.goto("/"); await resolveJourney(page);
...
test("discovery shows a dish concept and evidenced place", async ({ page }) => {
  await commonRoutes(page);
  await page.route("**/api/discovery/needs**", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ query: "mee pok", semantic_type: "dish", canonical_concept: "Mee pok", category: "japanese_food", confidence: "strong", related_terms: ["mee pok"], places: [{ display_name: "Ah Hoe Mee Pok", coordinate: { latitude: 1.38, longitude: 103.75 }, address: "Teck Whye", mall_or_hub: "Teck Whye Market", category: "japanese_food", confidence: "strong", evidence: ["mee pok"] }], live_fallback_used: false }) }));
  await page.goto("/");
  await page.getByRole("button", { name: /Browse categories/ }).click();
...
test("open-world compound errands remain plausible and reach optimization", async ({ page }) => {
  ...
  const openCompound = {
    ...
    required_errands: [
      { category: "open_meepok_1", required: true, substitutes_allowed: true, discovery_concept: "Meepok", discovery_category: "restaurants", open_need: { raw_text: "Meepok", inferred_type: "dish", resolution_status: "resolved" } },
      { category: "open_panadol_2", required: true, substitutes_allowed: true, discovery_concept: "panadol", discovery_category: "pharmacy", open_need: { raw_text: "panadol", inferred_type: "product", resolution_status: "resolved" } },
    ],
  };
  await page.route("**/api/intent/parse", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ status: "resolved", intent: openCompound, diagnostics: { discovered_need_count: 2 }, journey_mentions: [], journey_conflicts: [] }) }));
  await page.route("**/api/optimize-intent", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(result) }));
  await page.goto("/"); await resolveJourney(page);
...
test("product discovery labels category matches without claiming stock", async ({ page }) => {
  ...
  await page.route("**/api/discovery/needs**", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({
    query: "Panadol", semantic_type: "product", canonical_concept: "Panadol", category: "pharmacy", confidence: "likely", related_terms: ["panadol", "pharmacy"],
    places: [{ display_name: "Guardian", coordinate: { latitude: 1.304, longitude: 103.832 }, address: "Orchard Road", mall_or_hub: "ION Orchard", category: "pharmacy", confidence: "likely", evidence_tier: "supported", suitability: "category_likely", provenance: ["local_catalog"] }],
    warnings: ["These businesses are relevant to the product category; current item-level stock is not guaranteed."],
  }) }));
  await page.goto("/");
  await page.getByRole("button", { name: /Browse categories/ }).click();

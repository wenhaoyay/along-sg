...
test("different alternatives stay collapsed until requested", async ({ page }) => {
  ...
  await expect(page.locator(".alternative-tradeoff")).toContainText(/m (less|more) walking|min (less|more) travel/);
});
...
test("a mixed hours stop names each shop", async ({ page }) => {
  await commonRoutes(page);
  const mixed = { ...recommendation, stops: [{ ...recommendation.stops[0], businesses: [
    { ...recommendation.stops[0].businesses[0], opening_status: "open" },
    { ...recommendation.stops[0].businesses[1], opening_status: "unknown" },
  ] }] };
  await page.route("**/api/intent/parse", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ status: "resolved", intent, diagnostics: {}, journey_mentions: [], journey_conflicts: [] }) }));
  await page.route("**/api/optimize-intent", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ ...result, recommendations: { best_overall: mixed } }) }));
  await page.goto("/"); await resolveJourney(page);
  await page.getByLabel("What do you need on the way?").fill("KFC and bubble tea");
  await page.getByRole("button", { name: /Find best stop/ }).click();
  await expect(page.getByTestId("recommendation-sheet")).toBeVisible();
...
test("the theme button cycles auto, light and dark, and the choice survives a reload", async ({ page }) => {
  await commonRoutes(page);
  ...
  await expect(page.locator('meta[name="theme-color"]').first()).toHaveAttribute("content", "#0c1211");
